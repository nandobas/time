...
export default {
	...
	data() {
		return {
			...
			arAjudas: [{
					...
					texto: 'Define-se a atividade principal de uma empresa, a que mais contribui para geração do valor adicionado. O Valor Adicionado é o valor bruto da produção menos o custo das matérias-primas, bem como de outros consumos intermediários. Pode-se também definir como atividade econômica principal, aquela considerada de maior receita auferida ou esperada, dentre as constantes no ato constitutivo ou alterador.'
				},
				{
					titulo: 'CNAE Secundário',
					texto: 'Define-se a atividade secundária, como aquela destinada a terceiros, mas cujo valor adicionado é menor do que o da atividade principal. A maior parte das empresas exercem mais de uma atividade e, portanto, tem uma ou mais atividades secundárias relacionadas em seu ato constitutivo.'
				}
			]
		}
	},
	...
	methods: {
		...
		obterCnaes() {
			this.$root.$api.post('ListarCnae', {
				cnae: this.strFiltro
			}).then(
				(response) => {
					this.arCnaes = response.response;
					this.timeoutPesquisa = null;
					this.pagination.paginas = Math.round(this.arCnaes.length / 10);
				}
			)
		},
		pesquisar() {
		...
		verificaCnaeAdicionado(idcnae) {
			return this.$props.form.id_cnaes.some(id => id == idcnae);
		},
		adicionar(cnae) {
			...
			if (this.$props.form.cnaes.length == 3)
				this.zerarPesquisa();
		},
