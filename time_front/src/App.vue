...
<script>
    ...
    export default {
        ...
        watch: {
            $route (to, from){
                if (window.location.pathname.indexOf('/parametros') === -1 &&
        ...
        mounted: function () {
            ...
            if (localStorage.getItem('token')) {
                ...
                this.$root.$api.getUrlCompleta('http://www.econeteditora.com.br/acesso_spt_login.php?id=' + 
                    localStorage.getItem('HASH_USER')).then(response => {
                    if (response.blLogado) {
                        if (window.location.pathname.indexOf('/simulacao') === -1) {
                            this.$router.push({
                                name: 'homeLogado'
                            });
                            /*
