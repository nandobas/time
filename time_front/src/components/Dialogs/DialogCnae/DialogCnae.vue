<template>
	<v-dialog v-model="abrirModal" max-width="60%">
		<div class="image-header">
			<img class='icone' src="data:image/png;base64,">
		</div>

		<v-icon class="close" @click='fechar()'>mdi-close</v-icon>

		<v-card>
			<v-card-title class="headline">
				<div class="h1">
					{{ cnae.cnae + ' - ' + cnae.descricao }}
				</div>
			</v-card-title>
			<v-card-text class='pt-1'>
				<v-layout row wrap>
					<p>Compreeende</p>
					<div class='conteudo-compreende' v-html='cnae.compreende'></div>

					<p>Não compreende</p>
					<div class='conteudo-compreende' v-html='cnae.nao_compreende'></div>
				</v-layout>
			</v-card-text>
		</v-card>
	</v-dialog>
</template>
<script src="./DialogCnae.js"></script>
<style lang="scss">
	@import "./DialogCnae.scss";
</style>