...
export default {
	...
	mounted() {
		if (localStorage.getItem("lp_hash"))
			this.requisicao_hash = localStorage.getItem("lp_hash");
	...
	computed: {
		...
		calcularTotaisTrimestre() {
			...
			for (let i = 0; i <= 3; i++) {
				...
				for (let j = 0; j <= 2; j++) {
					if (obReceita.irpj) {
						vlrTotal += parseFloat(
							obReceita.irpj.irpj.mensal[trimestres[i][j]]
						);
					}

					if (obReceita.csll) {
						vlrTotal += parseFloat(
							obReceita.csll.csll.mensal[trimestres[i][j]]
						);
					}
				}
		...
		calcularTotaisPisCofins() {
			...
			for (let i = 1; i <= 12; i++) {
				...
				vlrTotal += obReceita.pis_cofins.pis ?
					parseFloat(obReceita.pis_cofins.pis.mensal[i]) :
					0;
				vlrTotal += obReceita.pis_cofins.cofins ?
					parseFloat(obReceita.pis_cofins.cofins.mensal[i]) :
					0;
		...
		calcularTotais() {
			...
			for (let i = 1; i <= 12; i++) {
				...
				let vlrTotal = 0;
				vlrTotal += obReceita.ipi.ipi ?
					parseFloat(obReceita.ipi.ipi.mensal[i]) :
					0;
				vlrTotal += obReceita.iss.iss ?
					parseFloat(obReceita.iss.iss.mensal[i]) :
					0;
				vlrTotal += obReceita.iss.icms ?
					parseFloat(obReceita.iss.icms.mensal[i]) :
					0;
	...
	methods: {
		setIndexAba(idxAba) {
			this.repositorio = "lucroPresumido";
		...
		calcular() {
			...
			if (this.requisicao_hash == requisicao_hash) {
				this.obDados = this.$props.preResult.result_lp;
				return true;
			}
			...
			this.$root.$api
				...
				.then(response => {
					...
					this.$props.preResult.result_lp = this.obDados;
		...
		obterImpostoMensal(strGrupo, strImposto, acumulado = true) {
			...
			if (acumulado === true) {
				let array = this.obDados.arReceitas.acumulado[strGrupo][strImposto]
					.acumulado;

				return Object.keys(array).map(mes => {
					if (strImposto == 'cpp') {
						let idcnae = this.$props.preResult.result_sn[this.intIndexCnae]
							.idcnae;

						return this.obDados.arReceitas.acumulado[strGrupo][strImposto][idcnae]
							.acumulado[mes];
					}
					return this.obDados.arReceitas.acumulado[strGrupo][strImposto]
						.acumulado[mes];
				});
			} else {
				let array = this.obDados.arReceitas.acumulado[strGrupo][strImposto]
		...
		obterImpostoTrimestral(strGrupo, strImposto) {
			let array = this.obDados.arReceitas.acumulado[strGrupo][strImposto]
				.trimestral;
			return Object.keys(array).map(trimestre => {
				return this.obDados.arReceitas.acumulado[strGrupo][strImposto]
											...
											arValidacoes.arMeses.forEach(
												(mes, idxMes) => {
													let Campo =
														...
														.arCampos[idxCampo];
													if (
														Campo.nota_adicao_bc_presumido &&
														(Campo.valor !=
															"R$ 0,00" &&
															Campo.valor != "0")
													) {
														if (
															...
															arMsg[
																Campo.nota_adicao_bc_presumido.id
															] =
															"<p> " +
															Campo
															.nota_adicao_bc_presumido
															.nota +
															"</p>";
