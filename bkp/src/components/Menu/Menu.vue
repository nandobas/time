	...
	<div id="menu">
		<v-navigation-drawer v-model="drawer" :mini-variant.sync="mini" hide-overlay stateless :app="true" :width="250">
			...
			<v-list dense class="pt-0 body">
				<div v-for="(item, index) in itemsMenu" :key="'menu'+index+item.title">
					<v-list-tile v-if="!item.subMenu || item.subMenu.length == 0 || mini" :class="[index == 0 ? 'active' : '', 'linha', index > 0 ? 'disabled' : '']" :to="item.url">
						<v-list-tile-action>
					...
					<v-list-group v-else :class="[index == 0 ? 'active' : '', 'group-linha']">
						<template v-slot:activator>
							<v-list-tile>
								...
								<v-list-tile-content>
									<v-list-tile-title ref="item_title">{{item.title}}</v-list-tile-title>
								</v-list-tile-content>
							</v-list-tile>
						</template>
						<v-list-tile :to="sub.url" v-for="(sub, indexSub) in item.subMenu" :key="'menu_sub'+indexSub+sub.title" class="linha ">
							<v-list-tile-title>
								<span class="pre"></span>
