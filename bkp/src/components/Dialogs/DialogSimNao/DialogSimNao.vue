<template>
    <v-dialog v-model="abrirModal" max-width="500px" content-class="dialog-sim-nao" persistent>
        <div class="image-header">
            <img
                class="icone"
                src="data:image/svg+xml;base64,
"
            />
        </div>

        <v-icon class="close" @click="fechar()">mdi-close</v-icon>

        <v-card>
            <v-card-title class="headline">
                <div class="h1">Atenção</div>
            </v-card-title>
            <v-card-text>
                <v-layout row wrap>
                    <div class="conteudo-ajuda" v-html="texto"></div>
                </v-layout>
            </v-card-text>
            <v-card-actions>
                <v-spacer></v-spacer>
                <div class="button-area">
                    <v-btn class="outline" @click="fechar(false);">
                        <span>Não</span>
                    </v-btn>
                    <v-btn @click="fechar(true)">
                        <span>Sim</span>
                    </v-btn>
                </div>
            </v-card-actions>
        </v-card>
    </v-dialog>
</template>
<script src="./DialogSimNao.js"></script>
<style lang="scss">
@import "./DialogSimNao.scss";
</style>