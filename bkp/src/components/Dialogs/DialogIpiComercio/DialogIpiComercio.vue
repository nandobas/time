<template>
    <v-dialog
        v-model="abrirModal"
        max-width="500px"
        content-class="dialog-atencao dialog-equiparacao"
        persistent
    >
            ...
            <v-card-text class="pb-0">
                <v-layout row wrap>
                    <v-flex class="text-md-center">
                        <v-flex class="mb-2">Equiparação a estabelecimento industrial para as seguintes atividades:</v-flex>
                        <v-layout row wrap class="item" v-for="cnae in cnaes" :key="cnae.idcnae">
                            <strong>{{ cnae.cnae + ' - ' + cnae.descricao }}</strong>
                            <v-flex class="switch-area">
                                <label
                                    aria-hidden="true"
                                    class="v-label theme--light v-label-left"
                                    style="left: 0px; right: auto; position: relative;"
                                >Não</label>
                                <v-switch v-model="cnae.blEquiparacao" color="primary" label="Sim"></v-switch>
                            </v-flex>
                        </v-layout>
                    </v-flex>
                </v-layout>
...
<script src="./DialogIpiComercio.js"></script>
<style lang="scss">
@import "./DialogIpiComercio.scss";
</style>