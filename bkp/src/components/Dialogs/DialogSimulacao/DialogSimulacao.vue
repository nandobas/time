    ...
    <v-dialog v-model="dialogNome" persistent max-width="350" content-class="dialog-simulacao">
        ...
        <v-card>
            <v-card-title class="headline">
                <div class="h1">{{strAcao}} Simulação</div>
            </v-card-title>
            <v-card-text class="pt-2 pb-2">
                <v-layout row wrap>
                    <div
                        class="text-xs-center text"
                    >Informe um nome para que você possa identificá-la posteriormente</div>
                    ...
                    <v-flex xs12 mt-3 mb-0 pb-0>
                        <v-text-field
                            ...
                            single-line
                            label="Nome"
                            v-model.trim="strNome"
                            ref="inputNome"
                            @keyup.enter="fecharDialogNome()"
                            class="pt-0"
                            outline
                            solo
                            :rules="[blValidarNome ? rules.required : false]"
                        ></v-text-field>
                    </v-flex>
                </v-layout>
            </v-card-text>
            <v-card-actions>
                ...
                <div class="button-area">
                    <v-btn
                        v-if="strAcao=='Copiar' || strAcao=='Salvar'"
                        class="outline"
                        @click="dialogNome = false;"
                    >
                        <span>Cancelar</span>
                    </v-btn>
